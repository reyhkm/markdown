...
`, 1)[0];
                        ...
                        J.test(h) || K.test(h) || Q.test(h) || W.test(t))
                            break;
...
const Se = /^(?: *(?:\n|$))+/
  ...
  , he = k(N).replace("hr", A).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", v).getRegex()
  ...
  , Be = {
    ...U,
    table: ie,
    paragraph: k(N).replace("hr", A).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", ie).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", v).getRegex()
}
  , ve = {
    ...U,
  ...
  , fe = k(/^!?\[(label)\]\[(ref)\]/).replace("label", P).replace("ref", H).getRegex()
  , ge = k(/^!?\[(ref)\](?:\[\])?/).replace("ref", H).getRegex()
...
class m {
    constructor(t) {
        ...
        e.inline = _.pedantic) : this.options.gfm && (e.block = q.gfm,
        this.options.breaks ? e.inline = _.breaks : e.inline = _.gfm),
            ...
            if (n.hooks) {
                ...
                for (const s in n.hooks) {
                    ...
                    const l = s
                      , o = n.hooks[l]
                      , p = r[l];
                    I.passThroughHooks.has(s) ? r[l] = c => {
